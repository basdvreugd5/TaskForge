    ...
    <main class="flex-1 p-6 lg:p-10">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-card-light dark:bg-card-dark rounded-xl w-full max-w-2xl shadow-xl">
                ...
                @if (session('success'))
                    <div class="p-4 mx-6 mt-4 bg-green-100 dark:bg-green-900/30 border border-green-200 dark:border-green-700 text-green-700 dark:text-green-300 rounded-lg font-medium" role="alert">
                        {{ session('success') }}
                    </div>
                @endif
                
                @if (session('error'))
                    <div class="p-4 mx-6 mt-4 bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-700 text-red-700 dark:text-red-300 rounded-lg font-medium" role="alert">
                        {{ session('error') }}
                    </div>
                @endif
                        ...
                        @foreach ($board->collaborators as $collaborator)
                            @php
                                ...
                                $roleClasses = [
                                    ...
                                    'editor' => 'bg-role-editor text-role-editor-text dark:bg-role-editor-dark dark:text-role-editor-text-dark',
                                    'viewer' => 'bg-role-viewer text-role-viewer-text dark:bg-role-viewer-dark dark:text-role-viewer-text-dark',
                                    'member' => 'bg-pastel-blue text-blue-600 dark:bg-blue-900/50 dark:text-blue-300', 
                                ];
                                
                                $roleClass = $roleClasses[$displayRole] ?? 'bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-300';
                            ...
                            <div class="flex items-center justify-between p-3 rounded-lg bg-background-light dark:bg-background-dark">
                                <div class="flex items-center gap-4">
                                    ...
                                    <img alt="{{ $collaborator->name }}'s avatar" 
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button class="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors" 
                                                    @if ($isOwner || $isCurrentUser) disabled @endif>
                                            <span class="material-symbols-outlined @if ($isOwner || $isCurrentUser) text-slate-400 dark:text-slate-600 @endif !text-xl">edit</span>
                                        </button>
                                        
                                        {{-- Remove Collaborator Button --}}
                                        {{-- Only allow removal if not the owner AND not the current user --}}
                                        @if (!$isOwner && !$isCurrentUser)
                                            <form method="POST" action="{{ route('dashboard.collaborators.destroy', [$board, $collaborator]) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-2 rounded-full hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors">
                                                    <span class="material-symbols-outlined text-red-500 !text-xl">delete</span>
                                                </button>
                                            </form>
                                        @else
                                            <button class="p-2 rounded-full" disabled>
                                                <span class="material-symbols-outlined text-slate-400 dark:text-slate-600 !text-xl">delete</span>
                                            </button>
                                        @endif
                                    </div>
                                </div>
                ...
                <div class="p-6 border-t border-slate-200 dark:border-slate-800">
                    ...
                    <form method="POST" action="{{ route('dashboard.collaborators.store', $board) }}">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            ...
                            <!-- Role Select -->
                            <div class="relative">
                                <select name="role" class="appearance-none w-full sm:w-auto bg-background-light dark:bg-background-dark border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 py-2.5 pl-3 pr-10 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition">
                            ...
                            <button type="submit" class="bg-primary text-white font-bold py-2.5 px-6 rounded-lg hover:bg-primary/90 transition-colors flex items-center justify-center gap-2">
